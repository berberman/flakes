...
fixedSN :: Text -> Text -> Text
fixedSN name fp = [trimming|with builtins; (fromJSON (readFile ./$fp)).$name|]
        ...
        T.unlines
          [ snippetN name version fetcher
            | (pkg@Pkg {..}, _) <- sha256sumsWithRecovered,
              let fetcher = sourceToFetcher pkg . Just $ fixedSN name (T.pack sha256Data)
          ]
